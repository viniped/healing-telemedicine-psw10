...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Já existe um cadastro de medico')
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medico.html',
                       {'consulta': consulta,'is_medico': is_medico(request.user),'documentos': documentos}) 
...
def finalizar_consulta(request, id_consulta):
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Somente.medicos podem acessar essa página.')
        return redirect('/usuarios/sair')
